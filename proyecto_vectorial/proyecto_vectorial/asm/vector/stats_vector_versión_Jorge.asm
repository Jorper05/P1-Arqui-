...
; Suma compensada mediante TwoSum, con todas las operaciones en float32.
; ymm0 acumula valores; ymm3 acumula errores de redondeo por carril.
; La reduccion y el remanente tambien conservan una correccion.
sum_array:
    xor     eax, eax
...
.sum_vec_loop:
    ...
    vaddps  ymm2, ymm0, ymm1 ; t = suma + valor
    vsubps  ymm4, ymm2, ymm0 ; z = t - suma
    vsubps  ymm5, ymm2, ymm4
    vsubps  ymm5, ymm0, ymm5 ; suma - (t - z)
    vsubps  ymm6, ymm1, ymm4 ; valor - z
    vaddps  ymm5, ymm5, ymm6 ; error de redondeo de la suma
    vaddps  ymm3, ymm3, ymm5 ; acumular la correccion
    vmovaps ymm0, ymm2
    add     eax, 8
    jmp     .sum_vec_loop

.sum_reduce:
    ; Guardar ocho sumas y ocho correcciones sin exigir alineacion de RSP.
    sub     rsp, 64
    vmovups [rsp], ymm0
    vmovups [rsp + 32], ymm3
...
.sum_reduce_loop:
    vmovss  xmm1, [rsp + rcx*4]
    vaddss  xmm2, xmm0, xmm1 ; t = suma + valor
    vsubss  xmm4, xmm2, xmm0 ; z = t - suma
    vsubss  xmm5, xmm2, xmm4
    vsubss  xmm5, xmm0, xmm5 ; suma - (t - z)
    vsubss  xmm6, xmm1, xmm4 ; valor - z
    vaddss  xmm5, xmm5, xmm6 ; error de redondeo de la suma
    vaddss  xmm3, xmm3, xmm5 ; acumular la correccion
    vmovaps xmm0, xmm2
    inc     ecx
    cmp     ecx, 16
    jl      .sum_reduce_loop
